...
if page == "Resources & Contact":
    ...
    # Form for scheduling a Zoom call
    st.markdown('''
    <div style="background-color: #F0F4F8; padding: 25px; border-radius: 12px; box-shadow: 0px 4px 15px rgba(0, 0, 0, 0.1); margin-top: 30px;">
        <h3 style="color: #1E2D39; text-align: center; font-weight: bold;">Schedule a Zoom Call with Us</h3>
        <p style="color: #4A4A4A; font-size: 16px; text-align: center;">Fill out the form below to schedule a call. We will get back to you with the details.</p>
    </div>
    ''', unsafe_allow_html=True)

    # Form to collect user information with mandatory fields
    with st.form(key='zoom_call_form'):
        name = st.text_input("Your Name *", placeholder="Enter your full name")
        phone = st.text_input("Your Phone Number *", placeholder="Enter your contact number")
        email = st.text_input("Your Email *", placeholder="Enter your email address")
        date = st.date_input("Select a Date for the Zoom Call *", value=None, min_value=datetime.date.today())

        # Hidden status field with default value
        status = "Pending"  # This will not be visible in the form but will be recorded in the CSV
    ...
    if submit_button:
        ...
        if not name or not phone or not email or not date:
            st.error("Please fill out all the fields marked with * before submitting the form.")
        else:
            ...
            csv_file_path = 'Clients_PMS_Data (30% sharing).csv'
            ...
            if os.path.exists(csv_file_path):
                ...
                # If it does not exist, create the file and write the data
                df.to_csv(csv_file_path, mode='w', header=True, index=False)

            # Optional: Display a success message to the user (without showing the data)
            st.success("Your request has been submitted successfully. We will get in touch with you soon.")
